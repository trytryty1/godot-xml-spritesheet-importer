...
var dir = DirAccess.open("res://image")
...
func _run() -> void:
	...
	var spritesheet: CompressedTexture2D = get_spritesheet_texture()
	...
	if spritesheet:
		...
		if open_spritesheet_data():
			...
			# loop through xml (not closing) elements
			while not end:
				edit_texture(spritesheet)

				end = not read_next_texture()
	else:
		print("could not load spritesheet")

##
# Create and return a stream texture from the spritesheet
##
func get_spritesheet_texture() -> CompressedTexture2D:
	var spritesheet: CompressedTexture2D = null
	var sprites_path: String = IMAGES_PATH + "/sprites.png"
	
	print("loading sprites from %s" % sprites_path)

	# check if spritesheet image exists and load it
	if dir.file_exists(sprites_path):
		print("exists")
		#var image = Image.load_from_file(sprites_path)
		#print(str(image))
		#spritesheet = ImageTexture.create_from_image(image)
		spritesheet = load("res://image/sprites.png")
		
		#spritesheet = ResourceLoader.load(sprites_path) as ImageTexture
		print(str(spritesheet))
		#spritesheet = load(sprites_path) as ImageTexture
...
##
# Open spritesheet xml data to get it ready to be read
##
...
##
func edit_texture(spritesheet: CompressedTexture2D) -> void:
...
func edit_tileset(texture: AtlasTexture, texture_name: String) -> void:
	...
	var begin: int = tileset_folder.rfind("/") + 1
	...
	if tileset_name == "tile":
		# if last subfolder name is just "tile" call the tileset simply "tileset"
	...
	var tileset_path: String = tileset_folder.path_join(tileset_name + ".tres")
	...
	if tileset:
		...
		if tile_id == -1:
			# tile doesn't exist, create a new one
